        ...
        enriched.append(
            {
                ...
                "available_date": score.available_date,
            }
        )
...
for e in extreme:
    print(
        f"  {e['code']} gap={e['gap']:+.2f} ea={e['ea']:+.4f} "
...
for q in range(5):
    ...
    print(
        f"Q{q + 1:1d} [{min(gv):+8.3f},{max(gv):+8.3f}] {len(subset):4d} "
