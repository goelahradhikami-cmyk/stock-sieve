...
class EvaluationEngine:
    ...
    def evaluate(self, research_decision_id: int,
                  horizon_days: int = 30,
                  evaluation_type: str = "ENTRY") -> EvaluationResult | None:
        ...
        if kline.empty or len(kline) < 3:
            ...
            return EvaluationResult(
                ...
                stock_return=0.0, market_return=0.0, sector_return=0.0,
    ...
    def save_to_db(self, result: EvaluationResult):
        ...
        c = conn.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?, 0, ?, ?, ?, ?, ?)
        """, (
            result.research_decision_id, result.horizon_days, result.eval_date,
            result.stock_return, result.market_return, result.sector_return,
            result.alpha_vs_market, result.alpha_vs_sector,
            result.max_drawdown_during, result.max_profit_during,
            1 if result.is_profitable else 0, 1 if result.alpha_positive else 0,
            result.verdict,
        ))
        eval_id = c.lastrowid
    ...
    def _get_genome_version(self, research_decision_id: int, created_at: str) -> str | None:
        ...
        """, (research_decision_id, created_at[:10] if created_at else "")).fetchone()
        conn.close()
    ...
    def _calculate_beta(self, kline: pd.DataFrame, start: date, end: date) -> float:
        ...
        try:
            ...
            stock_ret = stock_ret.clip(
                lower=-self.config.winsorize_clip,
                upper=self.config.winsorize_clip
            )
    ...
    def run_batch(self, research_decision_ids: list[int],
                   horizon_days: int = 30) -> list[EvaluationResult]:
