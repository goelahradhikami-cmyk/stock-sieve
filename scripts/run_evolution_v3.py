...
class ClimateProfile:
    ...
    def variance(self) -> float:
        return float(
            np.std([self.bull_alpha, self.bear_alpha, self.crash_alpha, self.sideway_alpha])
        )
    ...
    def best_climate(self) -> str:
        alphas = {
            "bull": self.bull_alpha,
            "bear": self.bear_alpha,
            "crash": self.crash_alpha,
            "sideway": self.sideway_alpha,
        }
...
class EvolutionArenaV3:
    ...
    def run(self, generations: int = 50, target_population: int = 32) -> list[GenerationHealthV3]:
        """Run multi-climate evolution."""
        population = self._initialize_population(target_population)
        print("=== Evolution Arena v3 (Multi-Climate) ===")
        ...
        for gen in range(generations + 1):
            ...
            survivors_info = self._climate_aware_selection(
                fitness_map, climate_profiles, population_counts
            )
            ...
            health = self._compute_health(
                gen, population, fitness_map, climate_profiles, children, extinct_ids
            )
    ...
    def _compute_climate_profiles(
        self, population: list[DoctrineGenome]
        ...
        try:
            rows = conn.execute(
                "SELECT doctrine_id, trade_date, residual_alpha FROM doctrine_fitness_history"
            ).fetchall()
        finally:
            conn.close()
        ...
        for row in rows:
            ...
            for climate, dates in CLIMATE_WORLDS.items():
                if date in dates:
                    if climate == "bull":
                        profiles[did].bull_alpha = (
                            (profiles[did].bull_alpha + alpha) / 2
                            if profiles[did].bull_alpha != 0
                            else alpha
                        )
                    elif climate == "bear":
                        profiles[did].bear_alpha = (
                            (profiles[did].bear_alpha + alpha) / 2
                            if profiles[did].bear_alpha != 0
                            else alpha
                        )
                    elif climate == "crash":
                        profiles[did].crash_alpha = (
                            (profiles[did].crash_alpha + alpha) / 2
                            if profiles[did].crash_alpha != 0
                            else alpha
                        )
                    elif climate == "sideway":
                        profiles[did].sideway_alpha = (
                            (profiles[did].sideway_alpha + alpha) / 2
                            if profiles[did].sideway_alpha != 0
                            else alpha
                        )
                    break
    ...
    def _climate_aware_selection(
        ...
        all_weather = [
            (did, p)
            for did, p in climate_profiles.items()
            if p.is_all_weather and did in fitness_map
        ]
        if all_weather:
    ...
    def _check_extinction(self, gen, fitness_map, population):
        ...
        for d in population:
            ...
            if (
                ...
                self.comp.record_death(
                    d.doctrine_id, gen, f"low_fitness_{self.consecutive_bad[d.doctrine_id]}_gens"
                )
        return extinct_ids
            ...
            for i in range(plan.num_children):
                ...
                if partners:
                    ...
                    if mutated:
                        self.registry.save(mutated)
                        family = (
                            mutated.doctrine_id.split("_")[0]
                            if "_" in mutated.doctrine_id
                            else mutated.doctrine_id
                        )
                        self.comp.record_birth(
                            mutated.doctrine_id, family, gen, "crossover_mutation"
                        )
    ...
    def _compute_health(
        self, gen, population, fitness_map, climate_profiles, children, extinct_ids
    ):
        ...
        specialists = {}
        for climate in ["bull", "bear", "crash", "sideway"]:
            specialists[climate] = sum(
                1
                for p in climate_profiles.values()
                if p.best_climate == climate and p.is_all_weather is False
            )
        avg_robustness = (
            float(np.mean([p.climate_robustness for p in climate_profiles.values()]))
            if climate_profiles
            else 0
        )
        ...
        all_weather_doctrines = [
            (did, p) for did, p in climate_profiles.items() if p.is_all_weather
        ]
        top_aw = (
            max(all_weather_doctrines, key=lambda x: x[1].min_alpha)[0]
            if all_weather_doctrines
            else ""
        )
    ...
    def _print_dashboard(self, h: GenerationHealthV3, elapsed: float):
        print(
            f"Gen {h.generation:3d} | Pop={h.population_size:2d} Fam={h.family_count} "
...
def main():
    ...
    print(
        f"Climate Robustness: {first.avg_climate_robustness:.3f} -> {last.avg_climate_robustness:.3f}"
    )
