...
Usage:
    python preview_child.py momentum_gen1_4727 momentum_chaser_v1 2026-06-15
"""
...
def preview_child_vs_parent(child_id, parent_id, signal_date_str):
    ...
    # Connect to evaluation.db (relative to project root)
    script_dir = os.path.dirname(os.path.abspath(__file__))
    db_path = os.path.join(script_dir, "data", "evaluation.db")
    ...
    if not child_row:
        ...
        print("   Run evolution first: python -c \"from src.evolution.engine_v1 import EvolutionEngineV1; EvolutionEngineV1(dry_run=False).run_cycle()\"")
        return
    if not parent_row:
    ...
    for family in ['quality', 'value', 'growth', 'momentum', 'risk', 'sentiment']:
        pw = parent_fm.get(family, {}).get('weight', 0) if isinstance(parent_fm.get(family), dict) else 0
        cw = child_fm.get(family, {}).get('weight', 0) if isinstance(child_fm.get(family), dict) else 0
        if abs(cw - pw) > 0.001:
            diff_count += 1
    ...
    parent_ret = calc_forward_return([s for s,_ in parent_picks[:5]], provider, signal_date, eval_date)
    child_ret = calc_forward_return([s for s,_ in child_picks[:5]], provider, signal_date, eval_date)
    ...
    for sid in stock_list:
        ...
        try:
            kline = provider.get_daily_kline(
                code, signal_date.isoformat(), signal_date.isoformat()
            )
            if kline.empty:
                continue
...
def calc_forward_return(picks, provider, signal_date, eval_date):
    ...
    for sid in picks:
        ...
        try:
            kline = provider.get_daily_kline(
                code, signal_date.isoformat(), eval_date.isoformat()
            )
