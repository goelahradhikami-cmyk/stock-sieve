...
from src.evolution.selection_guard import (
    EvolutionSelectionGuard, DoctrineFitnessInfo, BreedingPlan,
)
...
class EvolutionObservationArena:
    ...
    def run(self, generations: int = 5,
            backtest_dates: list[str] | None = None) -> dict:
        ...
        base_ids = ['deep_value_purist', 'aggressive_growth_hunter',
                    'momentum_trend_follower', 'deep_contrarian',
                    'dividend_income_compounder', 'quality_moat_compounder',
                    'quantitative_factor_rotator', 'smart_money_tracker']
        ...
        if len(population) < 8:
            ...
            for iv in self._base_identities():
                ...
                if d.doctrine_id in base_ids and d.doctrine_id not in [p.doctrine_id for p in population]:
                    reg.save(d)
                    population.append(d)

        print(f"=== Evolution Observation Arena ===")
        ...
        for gen in range(generations + 1):  # gen 0 = initial evaluation
            ...
            breeding_plans = self.guard.plan_breeding(survivors_info, target_population=len(population))

            # 5. Log survival decisions
            self._log_generation(gen, population, fitness_map, survivors_info, breeding_plans)

            # 6. Record trajectory
            gen_summary = self._summarize_generation(gen, population, fitness_map, survivors_info, breeding_plans)
            ...
            if ranked:
                print(f"Top:    {ranked[0].doctrine_id} (fitness={ranked[0].fitness:.3f}, quality={ranked[0].alpha_quality:+.4f})")
                print(f"Bottom: {ranked[-1].doctrine_id} (fitness={ranked[-1].fitness:.3f}, quality={ranked[-1].alpha_quality:+.4f})")
    ...
    def _record_ecology(self, gen: int, population: list[DoctrineGenome],
        ...
        for row in rows:
            ...
            if did not in doctrine_stats:
                doctrine_stats[did] = {
                    "factor_alphas": [], "selection_alphas": [],
        ...
        for did, info in sorted(ecology_map.items(), key=lambda x: -x[1]["crowding_score"]):
            ...
            print(f"  {did:36s}: crowd={info['crowding_score']:.2f} "
                  f"sel_alpha={info['selection_alpha']:+.4f} "
                  f"decay={decay_rate:+.2%} half_life={half_life}gen")

    def _breed(self, survivors: list[DoctrineGenome],
               plans: list[BreedingPlan]) -> list[DoctrineGenome]:
    ...
    def _log_generation(self, gen: int, population: list[DoctrineGenome],
                         fitness_map: dict, survivors_info: list,
                         breeding_plans: list) -> None:
        ...
        try:
            for d in population:
                ...
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    gen, d.doctrine_id, d.parent_doctrine_id,
                    children,
    ...
    def _summarize_generation(self, gen: int, population: list,
                               fitness_map: dict, survivors: list,
                               breeding_plans: list) -> dict:
...
def main():
    ...
    for gen_info in result["trajectory"]:
        ...
        print(f"  Population: {gen_info['population_size']} "
              f"(families: {gen_info['family_count']})")
        print(f"  Fitness: avg={gen_info['avg_fitness']:.3f} "
