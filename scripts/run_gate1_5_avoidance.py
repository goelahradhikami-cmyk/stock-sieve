...
def test_1_avoided_loss(episodes):
    ...
    high_ars = [
        e for e in episodes if e["confidence"] is not None and e["confidence"] >= q4_threshold
    ]
    low_ars = [
        e for e in episodes if e["confidence"] is not None and e["confidence"] < q1_threshold
    ]
    ...
    print(
        f"    market return: mean={s_high['mean_pct']:+.2f}%, median={s_high['median_pct']:+.2f}%",
        flush=True,
    )
    print(
        f"    correct block (mkt<0): {s_high['win_rate_pct']:.1f}% fell, {100 - s_high['win_rate_pct']:.1f}% rose",
        flush=True,
    )
    print(f"  Low ARS (Q1, conf < {q1_threshold:.1f}): N={s_low['n']}", flush=True)
    print(
        f"    market return: mean={s_low['mean_pct']:+.2f}%, median={s_low['median_pct']:+.2f}%",
        flush=True,
    )
    print(
        f"    correct block (mkt<0): {s_low['win_rate_pct']:.1f}% fell, {100 - s_low['win_rate_pct']:.1f}% rose",
        flush=True,
    )
    ...
    print(
        f"  Correct block rate: High ARS {100 - s_high['win_rate_pct']:.1f}% vs Low ARS {100 - s_low['win_rate_pct']:.1f}%",
        flush=True,
    )
    ...
    print("  Success criterion: delta <= -2.00pp (high ARS blocks are MORE correct)", flush=True)
    print(f"  Actual delta: {delta:+.2f}pp", flush=True)
    print(f"  VERDICT: {'PASS' if passed else 'FAIL'}", flush=True)
    if passed:
        print(
            "  ARS has risk information. High-ARS BLOCK episodes have more negative market returns.",
            flush=True,
        )
    else:
        print("  ARS does NOT rank risk. No point testing further.", flush=True)

    return {
        "n_high": s_high["n"],
        "n_low": s_low["n"],
        "high_mean": s_high["mean_pct"],
        "low_mean": s_low["mean_pct"],
        "delta": float(delta),
        "ci": [ci_lo, ci_hi],
...
def test_2_regret_reduction(episodes):
    ...
    low_ars_returns = np.array(
        [e["outcome"] for e in episodes if e["confidence"] < sorted_confs[len(sorted_confs) // 5]]
    )

    print("\n  --- Layer 1: Facts ---", flush=True)
    print(
        f"  Strategy A (buy all {len(all_returns)} blocked): mean={np.mean(all_returns) * 100:+.2f}%",
        flush=True,
    )
    print(
        f"  Strategy B (block high-ARS {len(high_ars_returns)}): avoided mean={np.mean(high_ars_returns) * 100:+.2f}%",
        flush=True,
    )
    print(
        f"  Strategy C (buy low-ARS {len(low_ars_returns)}): mean={np.mean(low_ars_returns) * 100:+.2f}%",
        flush=True,
    )
    ...
    print(
        "  If positive: high-ARS episodes have MORE pain (blocking them is more valuable)",
        flush=True,
    )
    ...
    print(f"  VERDICT: {'PASS' if passed else 'FAIL'}", flush=True)
    if passed:
        print("  Blocking high-ARS episodes avoids more pain than blocking low-ARS.", flush=True)
    else:
        print("  ARS does not prioritize pain avoidance.", flush=True)

    return {
        "ulcer_high": ui_high,
        "ulcer_low": ui_low,
        "ulcer_all": ui_all,
        "delta_ui": float(delta_ui),
        "verdict": "PASS" if passed else "FAIL",
    }
...
def test_3_crisis_robustness(episodes):
    ...
    for regime in sorted(by_regime.keys()):
        ...
        if len(h) >= 3 and len(l) >= 3:
            ...
            regime_results[regime] = {
                ...
                "n_high": len(h),
                "n_low": len(l),
                "high_mean": float(h_mean),
                "low_mean": float(l_mean),
                "delta": float(d),
                "ars_works": bool(ars_works),
            }
            print(
                f"  {regime:25s} {len(sub):5d} {h_mean:+7.2f}% {l_mean:+7.2f}% {d:+7.2f}pp {'✓' if ars_works else '✗'}",
                flush=True,
            )
        else:
    ...
    print(
        f"  Success: >= 2 regimes with ARS working. Actual: {positive_regimes}/{total_regimes}",
        flush=True,
    )
    print(f"  VERDICT: {'PASS' if passed else 'FAIL'}", flush=True)
...
def test_4_opportunity_cost(episodes):
    ...
    missed_upside = (
        float(np.mean([e["outcome"] for e in false_blocks]) * 100) if false_blocks else 0
    )
    overall_mean = float(np.mean(all_returns) * 100)

    print("\n  --- Layer 1: Facts ---", flush=True)
    print(
        f"  High-ARS blocks: {len(high_ars)}/{len(episodes)} ({block_rate * 100:.1f}% block rate)",
        flush=True,
    )
    ...
    print(
        f"  Missed upside < overall_mean + 3pp: {'PASS' if missed_upside < overall_mean + 3 else 'FAIL'}",
        flush=True,
    )
    print(
        f"  Block rate reasonable (< 30%): {'PASS' if block_rate < 0.3 else 'WARN'} ({block_rate * 100:.1f}%)",
        flush=True,
    )
    ...
    if catastrophic:
        print(
            "  CATASTROPHIC: FPR > 70%. ARS destroys more opportunity than it protects.",
            flush=True,
        )
    elif passed:
        print("  ARS reduces errors without excessive opportunity sacrifice.", flush=True)
    else:
        print(
            "  ARS over-filters. Protection destroys participation (Rule 11 violation).",
            flush=True,
        )
