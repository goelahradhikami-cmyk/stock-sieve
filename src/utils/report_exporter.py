...
class ReportExporter:
    ...
    def _export_committee_md(
        ...
        path = os.path.join(reports_dir, f"committee_report_{start_date}_to_{end_date}.md")
    ...
    def _export_committee_xlsx(
        ...
        try:
            ...
            return (
                "Error: pandas required for Excel export. Install with: pip install pandas openpyxl"
            )
        ...
        if not output_path:
            ...
            output_path = os.path.join(
                reports_dir, f"committee_report_{start_date}_to_{end_date}.xlsx"
            )
    ...
    def export_performance_report(
        ...
        if not output_path:
            ...
            os.makedirs(reports_dir, exist_ok=True)
            output_path = os.path.join(
                reports_dir, f"performance_report_{date.today().isoformat()}.xlsx"
            )
    ...
    def export_thesis_trace(
        ...
        os.makedirs(reports_dir, exist_ok=True)
        path = os.path.join(reports_dir, f"thesis_trace_{stock_code}_{date.today().isoformat()}.md")
        with open(path, "w", encoding="utf-8") as f:
            f.write(content)
        return path
    ...
    def export_evolution_log(self, format: str = "xlsx", output_path: str = None) -> str:
        ...
        if not output_path:
            ...
            os.makedirs(reports_dir, exist_ok=True)
            output_path = os.path.join(
                reports_dir, f"evolution_log_{date.today().isoformat()}.xlsx"
            )
