...
def test_classifier_stock_failure():
    ...
    eval_data = {
        'alpha_vs_market': -0.2, 'alpha_vs_sector': -0.15,
        'stock_return': -0.18, 'max_drawdown_during': 0,
        'net_return': -0.18,
    }
...
def test_classifier_market_failure():
    ...
    classifier = FailureClassifier()
    eval_data = {
        'alpha_vs_market': -0.20, 'stock_return': -0.15,
        'alpha_vs_sector': 0.0, 'max_drawdown_during': 0,
        'net_return': -0.15,
    }
...
def test_classifier_timing_early():
    ...
    classifier = FailureClassifier()
    eval_data = {
        'alpha_vs_market': 0.0, 'stock_return': 0.05,
        'max_drawdown_during': -0.25, 'net_return': 0.08,
    }
    failures = classifier.classify(eval_data, {})
    types = [f['type'] for f in failures]
...
def test_classifier_timing_late_exit():
    ...
    classifier = FailureClassifier()
    eval_data = {
        'alpha_vs_market': 0.0, 'stock_return': 0.0,
        'evaluation_type': 'EXIT', 'exit_opportunity_cost': 0.20,
        'max_drawdown_during': 0, 'net_return': 0,
    }
    failures = classifier.classify(eval_data, {})
    types = [f['type'] for f in failures]
...
def test_classifier_no_failure():
    ...
    classifier = FailureClassifier()
    eval_data = {
        'alpha_vs_market': 0.10, 'alpha_vs_sector': 0.05,
        'stock_return': 0.15, 'max_drawdown_during': -0.05,
        'net_return': 0.15,
    }
...
def test_engine_lifecycle():
    ...
    engine._save_candidate_rule({
        "rule_type": "test", "condition_json": "{}",
        "action_json": "{}", "confidence": 0.7, "source": "test",
    })
