...
class EvaluationCRUDMixin:
    ...
    def insert_research_decision(
        ...
        """,
            (
                ...
                thesis.get("pattern"),
                thesis.get("claim"),
                str(thesis.get("evidence", [])),
                thesis.get("catalyst"),
                str(thesis.get("invalidation", [])),
                thesis.get("horizon"),
                alpha_score,
                confidence,
                str(factor_snapshot),
    ...
    def get_agent_performance(
        self, conn, agent_id: str, period_type: str = "quarterly", limit: int = 8
    ) -> list[dict]:
        rows = conn.execute(
            """
        ...
        """,
            (agent_id, period_type, limit),
        ).fetchall()
        return [dict(r) for r in rows]

    @with_conn
    def get_genome_snapshot(self, conn, agent_id: str, status: str = "active") -> dict | None:
        row = conn.execute(
            """
    ...
    @with_conn
    def get_factor_memory(
        self, conn, factor_name: str, regime: str | None = None, limit: int = 20
    ) -> list[dict]:
        if regime:
            rows = conn.execute(
                """
    ...
    @with_conn
    def get_diversity_distance(self, conn, agent_id: str) -> float | None:
    ...
    @with_conn
    def get_post_mortems_with_mutations(
        self, conn, agent_id: str, lookback_months: int = 6
    ) -> list[dict]:
    ...
    def insert_committee_decision(self, conn, decision) -> int:
        ...
        """,
            (
                ...
                g(
                    "chairman_score", g("weighted_score")
                ),  # chairman score; system chairman composite = weighted_score, use independent chairman score if available in future
                g("weighted_score"),
                g("verdict"),
                g("verdict_reason"),
    ...
    @with_conn
    def get_committee_decision(self, conn, committee_id: str) -> dict | None:
        ...
        row = conn.execute(
            """
                ...
                WHERE rd.thesis_pattern = ?
                  AND er.evaluated_at >= date('now', ?)
            """,
            (thesis_pattern, f"-{months} months"),
        ).fetchone()
    ...
    def insert_portfolio_decision(
        ...
        market_risk_score: float | None = None,
    ...
    @with_conn
    def insert_portfolio_execution(
        self,
        conn,
        portfolio_decision_id: int,
        research_decision_id: int,
        agent_id: str,
    ...
    def get_latest_portfolio_state(self, conn, agent_id: str) -> dict:
        ...
        for r in rows:
            ...
            if action in ("BUY", "ADD"):
                ...
                if code in holdings:
                    avg_cost = (
                        holdings[code]["total_cost"] / holdings[code]["shares"]
