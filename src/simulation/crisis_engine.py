    ...
    def test_organism(self, organism: dict, scenario: dict) -> dict:
        ...
        for _day in range(days):
            daily_ret = np.random.normal(
                market_shock["equity_return"] / max(1, days / 20), daily_vol
            )
        ...
        max_dd = (
            min(
                ...
                for i, nav in enumerate(nav_path)
                if max(nav_path[: i + 1]) > 0
            )
            if nav_path
            else 0
        )
        ...
        for threshold, config in dd_response.items():
            ...
            if pct >= 25 and market["regime"] == "crisis":
                actions.append(
                    {
                        "type": action,
                        "target_exposure": min(0.3, exposure),
                ...
                actions.append(
                    {
                        "type": action,
                        "target_exposure": min(0.5, exposure),
