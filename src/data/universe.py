...
def scan_local_universe(vipdoc_root: str | None = None) -> list:
...
def tencent_batch_quotes(codes: list, batch_size: int = 60, pause: float = 0.1) -> dict:
...
def sync_security_master_from_local(
    vipdoc_root: str | None = None, db_path: str = "data/cache.db", enrich: bool = True
) -> int:
    ...
    if not rows:
        logger.warning(
            "❌ No local .day files found — check TDX vipdoc path "
    ...
    try:
        existing = {
            r[0]: r[1]
            for r in master.db.execute("SELECT code, name FROM security_master").fetchall()
        }
    except Exception as e:
        ...
        records.append(
            {
                ...
                "industry": "",
                "industry_index": None,
                "total_mv": q.get("mcap") or 0,
                "float_mv": q.get("float_mcap") or 0,
                "avg_turnover_20d": q.get("turnover_pct") or 0.0,
                "avg_amount_20d": 0.0,
                "is_st": is_st,
                "is_new_stock": 0,
            }
    ...
    stale = [
        ...
        for sid in (
            row[0]
            for row in master.db.execute("SELECT security_id FROM security_master").fetchall()
        )
        if sid not in fresh_ids
    ]
    if stale:
        master.db.executemany(
            "DELETE FROM security_master WHERE security_id=?", [(sid,) for sid in stale]
        )
        master.db.commit()
