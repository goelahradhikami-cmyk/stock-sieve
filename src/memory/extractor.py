...
class ExperienceExtractor:
    ...
    def _ensure_table(self):
        ...
        """)
        self.db.execute("CREATE INDEX IF NOT EXISTS idx_memory_agent ON investment_memory(agent_id)")
        self.db.execute("CREATE INDEX IF NOT EXISTS idx_memory_pattern ON investment_memory(thesis_pattern)")
        self.db.commit()
    ...
    def extract_daily(self, limit: int = 100) -> int:
        ...
        for row in rows:
            ...
            pm = self.db.execute(
                "SELECT error_type FROM post_mortems WHERE research_decision_id=?",
                (decision_id,)
            ).fetchone()
            ...
            # Extract key factors
            factors = json.loads(factors_raw) if isinstance(factors_raw, str) else (factors_raw or {})
            ...
            self.db.execute("""
                ...
                VALUES (?,?,?,?,?,?,?,?,?,?)
            """, (agent_id, decision_id, eval_id, thesis_pattern,
                  regime, horizon, json.dumps(key_factors),
                  alpha, success, error_type))
            count += 1
