...
"""
import sys, os, sqlite3, ast, hashlib, time
sys.path.insert(0, ".")
...
for code in CODES:
    ...
    fs = {
        "quality": res.quality_score, "value": res.value_score,
        "growth": res.growth_score, "momentum": res.momentum_score,
    }
    dh = hashlib.sha256(f"probe_agent|{code}|{PROBE_DATE}|{int(time.time()*1000)}".encode()).hexdigest()[:16]
    ih = hashlib.sha256(f"{code}|{PROBE_DATE}".encode()).hexdigest()[:16]
    rid = db.insert_research_decision(
        agent_id="probe_agent", genome_hash="probe", security_id=code,
        thesis={"thesis_id": f"auto_{code}_{PROBE_DATE}", "family": "value", "pattern": "probe",
                "claim": "probe", "evidence": [], "invalidation": [], "horizon": "12_months"},
        alpha_score=50, confidence=0.5, factor_snapshot=fs, risk_assessment={},
        entry_price=100, entry_date=PROBE_DATE, decision_hash=dh, input_hash=ih,
    )
...
for rid, code, g_exp in inserted:
    row = conn.execute("SELECT factor_snapshot FROM research_decisions WHERE id=?", (rid,)).fetchone()
...
print("\nRESULT:", "PASS — baostock multi-period growth flows into stored decisions (non-50)" if all_non50
