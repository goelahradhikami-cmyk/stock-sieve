...
class EvolutionEngineV1:
    ...
    def _get_next_cycle_id(self) -> int:
        row = self.db.execute(
            "SELECT MAX(cycle_id) FROM evolution_events"
        ).fetchone()
        return (row[0] or 0) + 1

    def _log_event(self, event_type: str, agent_id: str = None,
                    parent_id: str = None, description: str = "",
                    details: dict = None):
        self.db.execute("""
            INSERT INTO evolution_events
            (cycle_id, event_type, agent_id, parent_id, description, details_json)
            VALUES (?,?,?,?,?,?)
        """, (self.cycle_id, event_type, agent_id, parent_id,
    ...
    def _calculate_fitness(self, agent_id: str) -> AgentFitness | None:
        ...
        try:
            ...
            if len(regime_rows) >= 2:
                ...
                if alphas and np.mean(alphas) != 0:
                    robustness = max(0, min(1, 1 - (np.std(alphas) / (abs(np.mean(alphas)) + 0.01))))
                else:
                    robustness = 0.5
            else:
                robustness = 0.5
        ...
        return AgentFitness(
            ...
            fitness=fitness,
            avg_alpha=avg_alpha, win_rate=win_rate,
            avg_drawdown=avg_dd, sample_count=sample_count,
    ...
    def _get_identity(self, agent_id: str) -> np.ndarray:
        ...
        return np.array([
            dims.get('valuation', 0), dims.get('quality', 0),
            dims.get('growth', 0), dims.get('momentum', 0),
            dims.get('contrarian', 0), dims.get('patience', 0),
        ])
    ...
    def run_cycle(self, pending_mutations=None) -> dict:
        ...
        print(f"  Elites: {len(elites)} | Eliminated: {len(eliminated)} | Diversity-saved: {len(survivors)}")

        if self.dry_run:
            self._log_event("DRY_RUN", description=f"Would eliminate: {[a.agent_id for a in eliminated]}")
            self.db.commit()
            return {
                "cycle_id": self.cycle_id, "mode": "dry_run",
        ...
        for agent in eliminated:
            ...
            self._log_event("AGENT_FROZEN", agent.agent_id,
        ...
        for child in new_agents:
            ...
            if self._sandbox_validate(child, parent_score):
                ...
                self._log_event("AGENT_BORN", child['identity']['agent_id'],
                                parent_id=parent_id, description="Sandbox passed, activated")
            else:
                self._log_event("AGENT_REJECTED", child['identity']['agent_id'],
    ...
    def _crossover(self, a: AgentFitness, b: AgentFitness) -> dict:
        ...
        import yaml
        ga = yaml.safe_load(self.db.execute(
            "SELECT genome_yaml FROM agent_genome_snapshots WHERE agent_id=? AND status='active'",
            (a.agent_id,)).fetchone()[0]) or {}
        gb = yaml.safe_load(self.db.execute(
            "SELECT genome_yaml FROM agent_genome_snapshots WHERE agent_id=? AND status='active'",
            (b.agent_id,)).fetchone()[0]) or {}

        alpha = np.random.uniform(0.3, 0.7)
        child = ga.copy()
        child['identity'] = child.get('identity', {})
        child['identity']['agent_id'] = f"{a.strategy_genus}_gen{self.cycle_id}_{np.random.randint(1000,9999)}"
        child['identity']['generation'] = max(
        ...
        for dim in ['valuation', 'quality', 'growth', 'momentum', 'macro', 'contrarian', 'patience', 'concentration']:
            va = ga.get('investment_identity', {}).get('dimensions', {}).get(dim, 50)
            vb = gb.get('investment_identity', {}).get('dimensions', {}).get(dim, 50)
            child.setdefault('investment_identity', {}).setdefault('dimensions', {})[dim] = int(va * alpha + vb * (1 - alpha))
    ...
    def _mutate(self, genome: dict, pending_mutations=None) -> dict:
        ...
        """
        for family, config in genome.get('factor_model', {}).items():
