...
class AShareMicrostructure:
    ...
    def simulate_order_book(
        self, signal_price: float, quantity: int, action: str, adv: float
    ) -> dict:
...
class MarketEcologyEngine:
    ...
    def calc_capacity_limit(
        ...
        row = self.db.execute(
            "SELECT max_position_adv FROM capacity_genome WHERE genome_id=?", (genome_id,)
        ).fetchone()
        ...
        try:
            ...
            """,
                (
                    ...
                    action,
                    signal_price,
                    signal.get("market_price", signal_price),
...
class MarketRealityEngine:
    ...
    def simulate_order(
        ...
        stamp_duty = (
            turnover * self.config.get("stamp_duty", 0.001) if action in ("SELL", "REDUCE") else 0
        )
