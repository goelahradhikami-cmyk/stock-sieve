...
class MemoryRetriever:
    ...
    def search(
        ...
        current_vec = self._build_vector(
            factor_snapshot, market_regime, thesis_pattern, agent_identity
        )
        ...
        for row in candidates:
            ...
            mem_factors = (
                json.loads(factors_json) if isinstance(factors_json, str) else (factors_json or {})
            )
        ...
        if total_weight > 0:
            weighted_win_rate = (
                sum((1 if (c["success"] or 0) > 0 else 0) * c["decay_weight"] for c in scored)
                / total_weight
            )
        else:
        ...
        memory_risk_score = round(1.0 - weighted_win_rate, 2) if len(scored) >= 10 else 0.5
            ...
            pvec = (
                np.array(
                    [
                        ...
                        dims.get("quality", 50),
                        dims.get("growth", 50),
                        dims.get("momentum", 50),
                        dims.get("contrarian", 50),
                        dims.get("patience", 50),
                    ],
                    dtype=float,
                )
                / 100.0
