...
class IndexDataProvider:
    ...
    def sync_index(self, index_code: str, start_date: str, end_date: str):
        ...
        if df.empty:
            ...
            if n == 0:
                ...
                logger.info(
                    f"  {name}({index_code}): all history sources failed, using Tencent snapshot"
                )
    ...
    def _sync_tencent_history(self, index_code: str, start_date: str, end_date: str) -> int:
            ...
            try:
                ...
                records.append(
                    (
                        ...
                        float(low),
                        close_val,
                        close_val,  # adj_close == close (indices)
                        float(vol),
                        0.0,
                    )
                )
    ...
    def _sync_eastmoney_history(self, index_code: str, start_date: str, end_date: str) -> int:
            ...
            try:
                ...
                records.append(
                    (
                        ...
                        float(low),
                        close_val,
                        close_val,  # adj_close == close for indices
                        float(vol),
                        float(amt),
                    )
                )
    ...
    def sync_all(self, start_date: str = "2020-01-01", end_date: str | None = None):
        """Sync all preset indices."""
        if end_date is None:
            end_date = date.today().isoformat()
        total = 0
        for code, _name in self.INDEX_CODES.items():
            n = self.sync_index(code, start_date, end_date)
            total += n
        return total

    def _get_adj_close(self, index_code: str, trade_date: str) -> float | None:
