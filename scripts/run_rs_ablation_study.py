...
def _exp1_rs_quintile(rows):
    ...
    print("Test Hypothesis A: weak RS (price not yet reacted) > strong RS", flush=True)
    ...
    for q in range(5):
        ...
        _print_stats(f"Q{q + 1} (RS {min(rs_vals):+.4f} to {max(rs_vals):+.4f})", s)
    ...
    print(
        f"    Q1 residual: {q1['residual_mean']:+.4f} (positive {q1['positive_rate']:.1%})",
        flush=True,
    )
    print(
        f"    Q5 residual: {q5['residual_mean']:+.4f} (positive {q5['positive_rate']:.1%})",
        flush=True,
    )

    if q1["residual_mean"] is not None and q5["residual_mean"] is not None:
        if q1["residual_mean"] > q5["residual_mean"]:
            print("    ✅ Hypothesis A SUPPORTED: weak RS > strong RS", flush=True)
            verdict = "SUPPORTED"
        else:
            print("    ❌ Hypothesis A NOT supported: strong RS >= weak RS", flush=True)
...
def _exp2_threshold_sweep(rows):
    ...
    if no_gate["residual_mean"] and current["residual_mean"]:
        ...
        print(
            f"  No-gate vs RS>0: {no_gate['residual_mean']:+.4f} vs "
            f"{current['residual_mean']:+.4f} (delta {delta:+.4f})",
            flush=True,
        )
        if delta > 0:
            print("    ✅ Removing RS gate IMPROVES alpha", flush=True)
        else:
            print("    RS gate helps (alpha higher with gate)", flush=True)
...
def _exp3_frm_rs_matrix(rows):
    ...
    valid = [r for r in rows if r["relative_strength"] is not None and r["frm_score"] is not None]
    ...
    print(
        f"\n  FRM tertile thresholds: weak<{frm_t1:.1f}, medium<{frm_t2:.1f}, strong>={frm_t2:.1f}",
        flush=True,
    )
    print("\n  Residual Alpha Matrix (mean):", flush=True)
    print(
        f"    {'':10s} {'Q1(weak RS)':>14s} {'Q2':>14s} {'Q3':>14s} "
        f"{'Q4':>14s} {'Q5(strong)':>14s}",
        flush=True,
    )
    ...
    for key, cell in matrix.items():
        if cell["n"] >= 3:  # min sample
            ...
            if m > best_mean:
                ...
                best_cell = key
    if best_cell:
        print(f"\n  BEST CELL: {best_cell} (residual {best_mean:+.4f})", flush=True)

    return {
        "matrix": {
            k: {
                "n": v["n"],
                "residual_mean": float(np.mean(v["residuals"])) if v["n"] > 0 else None,
            }
...
def _exp4_early_late_rs(rows, conn):
    ...
    print("Test Hypothesis B: RS_60d (early) more predictive than RS_20d (late)", flush=True)
    ...
    for i, r in enumerate(sample):
        try:
            rs60 = scorer.compute(r["security_id"], r["trade_date"], lookback_days=60)
            results.append(
                {
    ...
    rs60 = np.array(
        [r["rs_60d"] for r in results if r["rs_60d"] is not None and r["rs_20d"] is not None]
    )
    ra = np.array([r["residual_alpha"] for r in results if r["rs_20d"] is not None])

    # Align
    aligned = [
        (r["rs_20d"], r["rs_60d"], r["residual_alpha"])
        for r in results
    ...
    for q in range(5):
        ...
        if len(subset_ra) > 0:
            print(
                f"    Q{q + 1} (RS60 {subset_rs.min():+.4f} to "
...
def _exp5_sample_composition(rows):
    ...
    print("Test Hypothesis D: Group B vs C differ by year/state/industry", flush=True)
    ...
    for lo, hi, label in [(0, 50, "weak(0-50)"), (50, 70, "med(50-70)"), (70, 100, "strong(70+)")]:
        b = [r for r in group_b if r["frm_score"] is not None and lo <= r["frm_score"] < hi]
        c = [r for r in group_c if r["frm_score"] is not None and lo <= r["frm_score"] < hi]
        b_ra = np.mean([r["residual_alpha"] for r in b]) if b else None
        c_ra = np.mean([r["residual_alpha"] for r in c]) if c else None
        b_str = f"{b_ra:+.4f}(N={len(b)})" if b_ra is not None else "n/a"
        c_str = f"{c_ra:+.4f}(N={len(c)})" if c_ra is not None else "n/a"
    ...
    for lo, hi, label in [(50, 70, "med"), (70, 100, "strong")]:
        ...
        if len(b) >= 3 and len(c) >= 3:
            ...
            delta = b_ra - c_ra
            print(
                f"    {label}: B={b_ra:+.4f}(N={len(b)}) "
                f"C={c_ra:+.4f}(N={len(c)}) delta={delta:+.4f}",
                flush=True,
            )
            if delta > 0:
...
def _export_report(results):
    ...
    lines.append("")
    lines.append("Test: does weak RS (price not yet reacted) produce higher alpha than strong RS?")
    lines.append("")
    if "quintiles" in e1:
        lines.append("| Quintile | RS Range | N | Residual Mean | Positive Rate |")
        lines.append("|----------|----------|---|---------------|---------------|")
        for q, data in e1["quintiles"].items():
            lines.append(
                f"| {q} | [{data['rs_range'][0]:+.4f}, "
    ...
    if "best_cell" in e3 and e3["best_cell"]:
        lines.append(f"**Best cell: {e3['best_cell']} (residual {e3['best_residual']:+.4f})**")
    lines.append("")
    ...
    if "corr_rs20_residual" in e4:
        lines.append(f"- Correlation RS_20d vs residual: {e4['corr_rs20_residual']:+.4f}")
        lines.append(f"- Correlation RS_60d vs residual: {e4['corr_rs60_residual']:+.4f}")
    ...
    lines.append("")
    lines.append(
        "See stdout for full analysis. Key finding determines "
