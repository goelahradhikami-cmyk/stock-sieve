...
def composite_result():
    ...
    financial = {
        "roe": 0.25, "roic": 0.18, "gross_margin": 0.65, "net_margin": 0.20,
        "pe_ttm": 18.5, "pb": 3.2, "fcf_yield": 0.05,
        "debt_to_equity": 0.8, "interest_coverage": 12.0,
        "revenue_growth_1y": 0.18, "earnings_growth_1y": 0.22,
    }
    fe = FactorEngine()
...
def test_import_evaluation_db():
    from src.data.evaluation_db import (
        EvaluationDB,
...
def test_import_committee_agent():
    from src.agents.committee_agent import (
        CommitteeAgent,
        CommitteeDecision,
        chairman_decision,
...
def test_import_post_mortem():
    from src.evaluation.post_mortem import PostMortemAnalyzer, ErrorCategory, ErrorSubtype
...
def test_database_tables(db):
    ...
    expected = [
        "agent_genome_snapshots", "agent_performance", "calibration_log",
        "committee_decisions", "decision_events", "evaluation_results",
        "factor_memory", "failure_patterns", "market_regime_snapshots",
        "portfolio_decisions", "post_mortems", "research_decisions",
        "thesis_outcomes",
    ]
...
def test_factor_engine_families():
    from src.factors.engine import FactorEngine
    fe = FactorEngine()
    assert len(fe.get_family_names()) == 6, f"expected 6 factor families, got {fe.get_family_names()}"
    assert len(fe.get_factor_names()) >= 25, f"expected >= 25 factors, got {len(fe.get_factor_names())}"
...
def test_research_agent_analyze(composite_result):
    ...
    stock = StockSnapshot(code="600519", name="贵州茅台", price=1680.0,
                          pe_ttm=30.0, pb=8.5, float_mcap=21000)
...
def _seed_post_mortem_data(db):
    ...
    conn.execute("""INSERT INTO research_decisions
        ...
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    """, (
        "value_purist_v1", "abc", "600519", "t1", "value", "quality_compound",
        "品牌护城河创造超额回报",
        ...
        "hash2", "input2", 1680.0, "2026-07-14",
    ))
...
def test_committee_review(db):
    ...
    class TSA:
        ...
        alpha_score = 7.5
        confidence = 8.0
        factor_profile = {"roe": 0.31, "gross_margin": 0.92,
                          "pe_percentile": 0.65, "fcf_yield": 0.04}
        risk_assessment = {"idiosyncratic_risk": "low", "sector": "消费",
...
def test_apply_committee_decision():
    ...
    approve_d = CommitteeDecision(
        committee_id="c1", research_decision_id=1, verdict="APPROVE",
        position_cap_modifier=1.0, confidence_modifier=0.0,
    )
