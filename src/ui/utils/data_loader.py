...
def _mock_committee_history() -> list[dict]:
    ...
    for i in range(8):
        ...
        ws = sum(scores[k] * [0.20, 0.20, 0.25, 0.15, 0.20][i] for i, k in enumerate(scores.keys()))
        ...
        mock.append(
            {
                ...
                "monitoring_flags_json": json.dumps(
                    ["MONITOR_VALUATION"] if "CONDITIONS" in v else []
                ),
...
def load_decision_timeline(
    _db=None, stock_code: str = "", limit: int = 5, days: int | None = None
) -> list[dict]:
    """Load decision chain for a stock, optionally limited to the last N days."""
    if _db is None or not stock_code:
        return (
            (_mock_decision_timeline(stock_code) if _is_demo_mode() else []) if stock_code else []
        )
    ...
    return [
        {
            ...
            "risk_assessment": json.dumps(
                {"idiosyncratic_risk": "low", "expected_drawdown_12m": 0.18}
            ),
            "stock_return": 0.08,
