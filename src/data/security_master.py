...
class SecurityMaster:
    ...
    def get_active_universe(self) -> pd.DataFrame:
        ...
        return pd.read_sql_query(
            "SELECT * FROM security_master WHERE status='active' AND is_st=0", self.db
        )

    def get_by_code(self, code: str) -> dict | None:
        """Look up a single stock by numeric code."""
        df = pd.read_sql_query(
            "SELECT * FROM security_master WHERE code=?", self.db, params=(code,)
        )
        return df.iloc[0].to_dict() if not df.empty else None

    def count(self) -> int:
        return self.db.execute("SELECT COUNT(*) FROM security_master").fetchone()[0]
