...
def get_equal_weight_return(
    eval_db: str, cache_db: str, trade_date: str, eval_date: str, local: LocalDataProvider
) -> float:
...
def style_neutral_alpha(
    portfolio_return: float,
    benchmark_return: float,
    portfolio_size_bias: float,
    portfolio_value_bias: float,
...
def main():
    ...
    results = {
        n: {b: {c: [] for c in CLIMATE_WORLDS} for b in ["L0_HS300", "L1_CSIAll", "L3_EqualWeight"]}
        for n in doctrines
    }
    ...
    for world_name, dates in CLIMATE_WORLDS.items():
        ...
        for d in dates:
            ...
            bench_equal = get_equal_weight_return(
                builder.eval_db_path, builder.cache_db_path, d, eval_date, local
            )
    ...
    for bench_name, bench_label in [
        ("L0_HS300", "L0 HS300"),
        ("L1_CSIAll", "L1 CSI All"),
        ("L3_EqualWeight", "L3 Equal Wt"),
    ]:
        print(f"\n--- {bench_label} ---")
        print(
            f"{'Doctrine':22s} {'Bull':>8s} {'Bear':>8s} {'Crash':>8s} {'Sideway':>8s} {'Min':>8s} {'AW?':>5s}"
        )
        print("-" * 75)
        aw_count = 0
        for dname in sorted(results.keys()):
            ...
            if is_aw:
                aw_count += 1
            print(
                f"{dname:22s} {scores['bull']:+8.2%} {scores['bear']:+8.2%} "
    ...
    for bench_name, bench_label in [
        ...
        interpretation = (
            "可能含 size/style 偏差"
            if bench_name == "L0_HS300"
            else "去掉大盘偏差"
            if bench_name == "L1_CSIAll"
            else "去掉全市场偏差"
        )
    ...
    aw_l0 = sum(
        ...
        for d in results
        if min(
            float(np.mean(results[d]["L0_HS300"][w])) if results[d]["L0_HS300"][w] else 0
            for w in ["bull", "bear", "crash", "sideway"]
        )
        > 0
    )
    aw_l1 = sum(
        ...
        for d in results
        if min(
            float(np.mean(results[d]["L1_CSIAll"][w])) if results[d]["L1_CSIAll"][w] else 0
            for w in ["bull", "bear", "crash", "sideway"]
        )
        > 0
    )
    aw_l3 = sum(
        ...
        for d in results
        if min(
            float(np.mean(results[d]["L3_EqualWeight"][w]))
            if results[d]["L3_EqualWeight"][w]
            else 0
            for w in ["bull", "bear", "crash", "sideway"]
        )
        > 0
