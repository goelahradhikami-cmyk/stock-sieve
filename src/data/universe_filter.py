...
class UniverseFilter:
    ...
    def filter(self, df: pd.DataFrame, target_date: date = None) -> pd.DataFrame:
    ...
    def _do_filter(self, df: pd.DataFrame, target_date: date = None) -> pd.DataFrame:
        ...
        if self.use_dynamic_liquidity and 'avg_amount_20d' in df.columns:
            if df['avg_amount_20d'].max() > 0:  # Only if real data present
                df = df[df['avg_amount_20d'] >= 1000]

        # ── Fix 4: Dynamic percentile threshold ───────────
        dynamic_threshold = self.min_avg_amount_20d
        if self.use_dynamic_liquidity and 'avg_amount_20d' in df.columns:
            if df['avg_amount_20d'].max() > 0:  # Only compute if real data present
                dynamic_threshold = max(
                    self.min_avg_amount_20d,
                    df['avg_amount_20d'].quantile(self.amount_percentile)
                )
        ...
        self.db.execute(
            "DELETE FROM universe_snapshot WHERE trade_date=?",
            (target_date.isoformat(),)
        )
        snapshot_records = [
            (target_date.isoformat(), sid, 'A', 0.0)
